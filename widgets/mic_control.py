...
class MicControl(QPushButton):
    ...
    # Инициализация кнопки микрофона: настройка размеров, загрузка ассетов и UI
    ...
    def _fallback(self, color: str) -> QIcon:
        ...
        qp.drawEllipse(int(center - 20), int(center - 30), 40, 40)
        qp.drawRect(int(center - 8), int(center), 16, 35)
        qp.drawEllipse(int(center - 15), int(center + 25), 30, 10)
        qp.end()
